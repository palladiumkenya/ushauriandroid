...
//        requestQueue.add(stringRequest);
    ...
    public void sendClientEncounter(final String msg, final String phone) {
        try{
            List<UrlTable> _url =UrlTable.findWithQuery(UrlTable.class, "SELECT *from URL_TABLE ORDER BY id DESC LIMIT 1");
            if (_url.size()==1){
                for (int x=0; x<_url.size(); x++){
        ...
        pr.showProgress("Sending client encounter details.....");
        final int[] mStatusCode = new int[1];

        StringRequest stringRequest = new StringRequest(Request.Method.POST, z+Config.SENDENCOUNTERTO_DB,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
//                        Toast.makeText(ctx, "message "+response, Toast.LENGTH_SHORT).show();
